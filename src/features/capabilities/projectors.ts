...
import type { ToolsFeatureState } from './tools-machine.ts'
import type { McpFeatureState } from './mcp-machine.ts'
...
function matchesSkill(entry: SessionSkillEntry, words: readonly string[]): boolean {
...
export function projectSkillsCatalog(
  ...
  return Object.freeze({
    rows: Object.freeze(snapshot.skills.filter(entry => matchesSkill(entry, words))),
...
export function detachMcpToolsSnapshot(snapshot: SessionToolsSnapshot): SessionToolsSnapshot {
  return detachToolsSnapshot(snapshot)
}
