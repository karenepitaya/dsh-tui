...
describeOnWindows('official DeepSeek Harness profile release gate', () => {
  it('runs DSH-TUI through the repo-local mock provider and a real ConPTY', async () => {
    ...
    expect(stdout).toContain('standard_toolchain=catalog-25+calls-12+approval-allow-reject+question-answer-cancel')
    expect(stdout).toContain('toolchain_model_requests=13')
