...
describeOnWindows('official DeepSeek Harness profile release gate', () => {
  it('runs DSH-TUI through the repo-local mock provider and a real ConPTY', async () => {
    ...
    expect(stdout).toContain('status=official-token-meter+request-recovery+model-route')
    ...
    expect(stdout).toContain('tool_directory=exact-agent-27+read-only+model-requests-0')
    expect(stdout).toContain('capabilities=alias-opened+categories+skills-detail-modal+tools-schema-modal+mcp-empty')
    ...
    expect(stdout).toContain('standard_toolchain=catalog-26+calls-15+approval-allow-reject+question-answer-cancel+goal-action-pause-resume-pause+plan-review-approve+job-run-kill')
    expect(stdout).toContain('workbench=goal-active-paused-active-paused+plan-on-review-off+todo-live+activity-live-killed')
    expect(stdout).toContain('toolchain_model_requests=15')
    expect(stdout).toContain('toolchain_retry_requests=1')
    expect(stdout).toContain('request_recovery=official-retry+statusline+status-workspace')
    expect(stdout).toContain('request_route=official-header-context+status-workspace')
