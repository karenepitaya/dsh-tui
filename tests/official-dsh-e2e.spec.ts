...
describeOnWindows('official DeepSeek Harness profile release gate', () => {
  it('runs DSH-TUI through the repo-local mock provider and a real ConPTY', async () => {
    ...
    expect(stdout).toContain('request_recovery=official-retry+statusline+fixed-attempt-overlay')
    expect(stdout).toContain('request_route=official-header-context+fixed-route-overlay')
