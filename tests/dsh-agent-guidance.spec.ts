...
async function host(): Promise<Context> {
  ...
  await ctx.plugin(SystemPrompt, { personaPrefix: 'Selected persona.' })
  return ctx
}
...
function scopedAgent(ctx: Context, id: string) {
  ...
  // Harness 0.1.5 removed Context.agent; the unpublished Agent IS the scope
  // key, so the scoped context alone is the exact-Agent channel.
...
describe('DSH Agent guidance official system-prompt contract', () => {
  it('keeps guidance and per-step time independent across two hosts, complete prompts, and unload', async () => {
    ...
    installDshAgentGuidance(firstOwner.agent.ctx, firstOwner.agent)
    installDshAgentGuidance(secondOwner.agent.ctx, secondOwner.agent)
  ...
  it('prepares before publication, isolates exact Agent scopes, and removes only owned guidance', async () => {
    ...
    const prepared = await attempt.setup(first.agent.ctx, first.agent)
    ...
    expect(firstAssembly.sections.map(section => section.name)).toEqual([
      'harness:identity', 'deployment:persona-prefix', 'dsh-tui:agent-guidance',
    ...
    expect((await ctx.systemPrompt.assemble(assembleContextFor(second.agent))).sections)
      .not.toContainEqual(expect.objectContaining({ name: DSH_AGENT_GUIDANCE.name }))
    const disposeSecond = installDshAgentGuidance(second.agent.ctx, second.agent)!
    prepared.commit()
    ...
    installDshAgentGuidance(resumed.agent.ctx, resumed.agent)
  ...
  it('respects an official complete prompt and keeps no registration after setup fails', async () => {
    ...
    await expect(attempt.setup(owner.agent.ctx, owner.agent)).rejects.toThrow('setup failed')
    // If rollback leaked guidance, this exact-name registration would throw.
    expect(() => installDshAgentGuidance(owner.agent.ctx, owner.agent)).not.toThrow()
    expect(renderPrompt(await ctx.systemPrompt.assemble(assembleContextFor(owner.agent))))
      .toBe('User owns the complete prompt.')
  })
  ...
  it('cannot turn a host, untagged Agent, or another scoped context into global guidance', async () => {
    ...
    expect(installDshAgentGuidance(bare, first.agent)).toBeUndefined()
    expect(() => installDshAgentGuidance(ctx, first.agent)).toThrow('exact Agent scope')
    expect(() => installDshAgentGuidance(ctx.extend({}), first.agent))
      .toThrow('exact Agent scope')
    expect(() => installDshAgentGuidance(first.agent.ctx.extend({}), second.agent))
      .toThrow('exact Agent scope')
    expect((await ctx.systemPrompt.assemble()).sections).toHaveLength(3)
  })
})
