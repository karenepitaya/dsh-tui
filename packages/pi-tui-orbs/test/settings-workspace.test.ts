...
const model: SettingsWorkspaceModel = {
  header: "DSH 设置", title: "外观与交互", height: 30, subtitle: "调整显示与操作方式", scope: "用户设置",
  categories: [
...
describe("SettingsWorkspace", () => {
  ...
  it.each([
    ...
    const mono = new SettingsWorkspace({ ...model, height: 24, modal }).render(80);
  ...
  it.each([[40, 12], [80, 24], [160, 50]])("places modal help below the panel at the bottom of %sx%s", (width, height) => {
    ...
    const modals: NonNullable<SettingsWorkspaceModel["modal"]>[] = [
    ...
    for (const modal of modals) {
      const workspace = new SettingsWorkspace({ ...model, height, modal });
  ...
  it("paints badges independently from focus and retains their meaning in mono", () => {
    ...
    const text = new SettingsWorkspace({ ...model, height: 12, modal }, theme).render(40).join("\n");
    ...
    const home = new SettingsWorkspace(homeModel, theme).render(40).join("\n");
    expect(home).toContain("已配置");
    expect(paints.some((call) => call.role === "success" && call.text.includes("已配置"))).toBe(true);
    expect(paints.some((call) => call.role === "focus" && call.text.includes("已配置"))).toBe(false);
    const mono = new SettingsWorkspace(homeModel).render(40).join("\n");
  ...
  it("uses the complete footer height in the confirmation fit gate", () => {
    ...
    const height = Array.from({ length: 50 }, (_, index) => index + 1).find((rows) => fitsSettingsConfirmation(40, rows, modal))!;
    const lines = new SettingsWorkspace({ ...model, height, modal }).render(40);
    const bottom = lines.findIndex((line) => line.includes("└"));
    ...
    expect(fitsSettingsConfirmation(40, height - 1, modal)).toBe(false);
  });

  it.each([[40, 12], [80, 24], [160, 40]])("renders a directory home without save controls or search at %sx%s", (width, height) => {
    const lines = new SettingsWorkspace({ ...model, height, actions: [], searchHidden: true,
  ...
  it("uses caller actions and preserves their disabled state", () => {
    ...
    const text = new SettingsWorkspace({ ...model, actions: [{ id: "add", label: "添加提供商", disabled: true }, { id: "back", label: "返回" }] }, theme).render(80).join("\n");
  ...
  it.each([[40, 12], [80, 24], [160, 40]])("keeps immediate actions free of implicit saved status at %sx%s", (width, height) => {
    ...
    const text = new SettingsWorkspace(data).render(width).join("\n");
    expect(text).toContain("添加提供商");
    expect(text).not.toContain("已保存");
    expect(new SettingsWorkspace({ ...data, dirtyCount: 2 }).render(width).join("\n")).toContain("2 项未保存");
    expect(new SettingsWorkspace({ ...data, message: "设置已更新" }).render(width).join("\n")).toContain("设置已更新");
    expect(new SettingsWorkspace({ ...model, height, dirtyCount: 0 }).render(width).join("\n")).toContain("已保存");
  });
  ...
  it.each(["color", "mono"])("gives both on and off switches a non-color focus cue in a %s terminal", async (mode) => {
    ...
    for (const checked of [false, true]) {
      ...
      for (const focus of ["navigation", "content"] as const) {
        const lines = new SettingsWorkspace({ ...data, focus }, theme).render(80);
  ...
  it("shows a successful save notice exactly once instead of duplicating the generic saved state", () => {
    const text = new SettingsWorkspace({ ...model, dirtyCount: 0, message: "所有更改已保存" }).render(120).join("\n");
  ...
  it.each([[200, 30], [160, 30], [120, 30], [80, 24], [40, 12], [80, 6]])("fits %sx%s while preserving the current field and save/exit feedback", (width, height) => {
    const workspace = new SettingsWorkspace({ ...model, height });
    const lines = workspace.render(width);
  ...
  it("keeps descriptions immediately beneath labels, with visible select, segmented and toggle surfaces", () => {
    const lines = new SettingsWorkspace(model).render(120);
  ...
  it("scrolls the selected field into view after model changes and resize", () => {
    ...
    const workspace = new SettingsWorkspace({ ...model, height: 12, selectedFieldId: "field-0", groups: [{ id: "all", title: "全部", fields }] });
    workspace.render(80);
  ...
  it("never offers confirmation when the full warning and actions cannot fit", () => {
    const modal = { kind: "confirmation" as const, title: "扩大权限", lines: ["确认后将允许修改工作区以外的文件。".repeat(5)], actions: [{ id: "cancel", label: "取消" }, { id: "allow", label: "确认扩大权限" }], selectedIndex: 1 };
    expect(fitsSettingsConfirmation(40, 6, modal)).toBe(false);
    const text = new SettingsWorkspace({ ...model, height: 6, modal }).render(40).join("\n");
    expect(text).toContain("取消");
    expect(text).toContain("放大");
    expect(text).not.toContain("确认扩大权限");
    expect(fitsSettingsConfirmation(120, 30, modal)).toBe(true);
  });
  ...
  it("keeps CJK/emoji editor and search cursors on their actual visible character after horizontal scrolling", () => {
    ...
    const workspace = new SettingsWorkspace({ ...model, height: 12, modal: { kind: "editor", title: "修改", text, cursor } });
  ...
  it("keeps labels outside the active control color and preserves text when a theme is injected", () => {
    const workspace = new SettingsWorkspace(model, { paint: (role, text) => role === "focus" ? `\x1b[48;2;30;90;70m${text}\x1b[49m` : text });
    ...
    expect(lines.map(stripTerminalSequences)).toEqual(new SettingsWorkspace(model).render(120));
  ...
  it("wraps complete descriptions/errors and hides pending input cursors", () => {
    ...
    const workspace = new SettingsWorkspace({ ...model, height: 30, message, groups: [{ id: "a", title: "连接", fields: [{ id: "theme", label: "主机", description, control: { kind: "text", value: "localhost" } }] }] });
    const rendered = workspace.render(100).join("\n");
    expect(rendered).toContain("说明末尾");
    expect(rendered.replace(/[\s│]/gu, "")).toContain("恢复建议末尾");
    for (const overlay of [{ focus: "search" as const, search: { text: "abc", cursor: 2 } }, { modal: { kind: "editor" as const, title: "编辑", text: "abc", cursor: 2 } }]) {
  ...
  it("sanitizes control sequences and preserves read-only, pending and error states", () => {
    const text = new SettingsWorkspace({ ...model, writable: false, disabledReason: "只读配置", pending: true,
