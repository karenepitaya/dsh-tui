...
> 当前接手入口（2026-09-08）：先读 [控件收拢与导航性能记录](UI-CONTROL-REUSE-AND-NAVIGATION-2026-09-08.md)。共享 Button / SelectionList 及旧页面选择行迁移已实施，并修复 Settings 重复布局、Provider 导航重复读取快照及连续 `j/k` 输入遗漏。当前工作基于 `3861d88` 继续，保留本轮未提交修改；[UI 控件复用审计](UI-CONTROL-REUSE-AUDIT-2026-09-08.md) 中的数量是修复前统计。下列“未 commit / push”和脏工作区描述均是各阶段历史快照，当前状态以 Git 为准。
